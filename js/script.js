...
function countdownTimer(endDate, selector) {
  ...
  function update() {
    ...
    if (distance < 0) { container.innerHTML = "EXPIRED"; clearInterval(interval); return; }
...
document.querySelectorAll(".product-card button.add-to-cart").forEach(btn => {
  btn.addEventListener("click", (e) => {
    ...
    popupAddBtn.onclick = () => {
      ...
      if (purpose) addToCart({ title: popupTitle.textContent, price: card.querySelector("p").textContent.replace(/[^0-9]/g,''), size: sizeSelect.value });
...
cartToggleBtns.forEach(btn=>btn.addEventListener("click",()=>cartPanel.style.right="0"));
